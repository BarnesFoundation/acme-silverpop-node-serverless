...
export async function processToRecords(
  ...
  for (const element of resultsList) {
    ...
    if (
      reportType === ReportEnums.MEMBERSHIP_REPORT &&
      element.fieldName === "MembershipExternalMembershipId"
    ) {
      const { encryptedLogInValues, encryptedRenewValues, linkExpValues } =
  ...
  switch (objectType) {
    ...
    case ReportEnums.TRANSACTION_REPORT:
      return new Transaction(
        r.OrganizationName,
        r.OrganizationCategoryName,
        r.TransactionAmount,
        r.DiscountedTransactionAmount,
        r.DiscountTransactionValue,
        r.SaleChannel,
        r.OrderItemType,
        r.ItemName,
        r.CouponCode,
        ...
        r.AddOn,
        r.Quantity,
        r.DiscountedUnitPrice,
        r.PaymentAmount,
        r.Email,
        r.EventTemplateCustomField2,
        r.TransactionId,
        r.OrderNumber,
        "",
        "",
    ...
    case ReportEnums.MEMBERSHIP_REPORT:
      return new Membership({
        ...
        MembershipLevelName: r.MembershipLevelName,
        MembershipOfferingName: r.MembershipOfferingName,
        MembershipSource: r.MembershipSource,
        MembershipExternalMembershipId: r.MembershipExternalMembershipId,
        MembershipJoinDate: r.MembershipJoinDate,
        MembershipStartDate: r.MembershipStartDate,
        MembershipExpirationDate: r.MembershipExpirationDate,
        MembershipDuration: r.MembershipDuration,
        MembershipStanding: r.MembershipStanding,
        MembershipIsGifted: r.MembershipIsGifted,
        RE_MembershipProgramName: r.RE_MembershipProgramName,
        RE_MembershipCategoryName: r.RE_MembershipCategoryName,
        RE_MembershipFund: r.RE_MembershipFund,
        RE_MembershipCampaign: r.RE_MembershipCampaign,
        RE_MembershipAppeal: r.RE_MembershipAppeal,
        CardType: r.CardType,
        CardName: r.CardName,
        CardStartDate: r.CardStartDate,
        CardExpirationDate: r.CardExpirationDate,
        CardCustomerPrimaryCity: r.CardCustomerPrimaryCity,
        CardCustomerPrimaryState: r.CardCustomerPrimaryState,
        CardCustomerPrimaryZip: r.CardCustomerPrimaryZip,
        CardCustomerEmail: r.CardCustomerEmail,
        CardCustomerFirstName: r.CardCustomerFirstName,
        CardCustomerLastName: r.CardCustomerLastName,
        LogInLink: r.LogInLink,
        RenewLink: r.RenewLink,
        LinkExp: r.LinkExp,
      });
...
export async function generateMembershipLinks(values) {
  ...
  const renewLinkValues = values.map((value) =>
    [value, unixExpiry, "/renew"].join(",")
  );
  ...
  try {
    ...
    console.log(
      "Error in PayloadProcessor.generateMembershipLinks: Could not encrypt member links due to",
      e
    );
  }
...
export async function batchEncrypt(
  ...
  if (remainder.length) {
    ...
    return await Promise.allSettled(promiseArray)
      ...
      .catch((e) => {
        console.log(
          "Error in PayloadProcessor.batchEncrypt: Could not encrypt member links due to",
          e
        );
        return [];
      });
